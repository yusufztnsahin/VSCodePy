# region fonksiyon_tanimlama
"""
1→ def ile başlar
2→ fonksiyonun adı yazılır
3→ aç kapa parantez
4→ : iki nokta üst üste
def mesaj():
    print("lütfen bir sayı giriniz: ")
mesaj()
s1 = int(input())
mesaj()
s2 = int(input())
mesaj()
s3 = int(input())
print(f"{(s1+s2+s3)/3}")
"""
# endregion

# region fonksiyon_cagirma
"""
print("a")
def welcome():
    print("b")
print("c")
welcome()
print("d")
"""
# endregion

# region fonksiyon_duplicate
"""
def welcome():
    print("ilk f.")
def welcome():
    print("ikinci f.")
welcome()
"""
# endregion

# region fonksiyon_pass
"""
#pass???
def welcome():
    pass
"""
# endregion

# region fonksiyon_turleri
"""
1→ Parametre Almayan, Değer Döndürmeyen
2→ Parametre Alan, Değer Döndürmeyen
3→ Parametre Almayan, Değer Döndüren
4→ Parametre Alan, Değer Döndüren
"""
# endregion

"""
Kullanıcı Adınızı Giriniz: admin  #fonk. içinde input
Hoşgeldiniz Admin
"""

"""
def kAdi():
    mesaj = input("Kullanıcı Adınızı Giriniz: ")
    if mesaj == ("Admin"):
        print(f"Hoşgeldiniz {mesaj}")
    else:
        print(f"{mesaj} hesabı ile yönetim paneline giremezsiniz")


kAdi()
"""
